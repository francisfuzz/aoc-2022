...
let lastNum = 0;
let remainder = 0;
// init an array of false values, one for each board
const bingoBoards = new Array(boards.length).fill(false);

for (let i = 0; i < numbers.length; i++) {
    lastNum = numbers[i];
    console.log(`Calling number ${lastNum}`);
    
    let grandBingo = false;
    for(let board of boardsObj) {
        if(board.mark(lastNum)) {
            bingoBoards[board.id] = true;
            remainder = board.remainderSum();
            if (bingoBoards.every(b => b)) {
                console.log(`Last board to win is ${board.id}`);
                grandBingo = true;
                break;
            } else {
                console.log(`Number boards remaining: ${bingoBoards.filter(b => !b).length}`);
            }
        }
    }
